...
MotionData IMU::MotionModel(double t)
{
    ...
    */
    ...
    //Eigen::Vector3d eulerAngles(k_roll, k_pitch, K);
...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    // TODO: start from the first measurement
    for (int i = 1; i < imudata.size(); ++i) { //for (int i = 1; i < imudata.size(); ++i) {
        ...
        // TODO: donot change the order of calculation steps
        // We start IMU's data from time stap 0, 
        // so we update acc. first and then update quaternion.
        // Furtuhermore, using Rwb or Qwb to update rotation are identical.
        // TODO: normalize quaternion.
        Eigen::Vector3d acc_w = Qwb * (imupose.imu_acc) + gw;
        Eigen::Vector3d acc_w = Qwb * (imupose.imu_acc) + gw;  // aw = Rwb * ( acc_body - acc_bias ) + gw
        Qwb = (Qwb * dq).normalized();
        Vw = Vw + acc_w * dt;
        Pwb = Pwb + Vw * dt + 0.5 * dt * dt * acc_w;  //Qwb = Qwb * dq;
